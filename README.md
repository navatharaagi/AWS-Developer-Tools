...
- AWS Console-->CodePipeline-->select the pipeline that we want to add the Action to i.e.,"JenkinsPipeline"-->Edit-->Click on "Stage" at the bottom to add a new stage-->Name "Production"-->Click on "Action" to add a new Action--> select “Action category” as “Deploy”-->Give Action Name as "ProductionDeploy"-->select “Approval type” as “Manual approval”--> SNS topic ARN” select the SNS Topic that our reviewer is subscribed to i.e,"TriggerTopic"--> For “URL for review”,[goto EC2 instance & select "CodeDeployTestInstance",Copy Public IP of it] http://<Paste the copied Public IP>/index.html--> click "Add action"-->Save Pipeline changes.
