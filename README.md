...
- AWS Console-->CodePipeline-->select the pipeline that we want to add the Action to i.e.,"JenkinsPipeline"-->Edit-->Click on "Stage" at the bottom to add a new stage-->Name "Approval"-->Click on "Action" to add a new Action--> select “Action category” as “Approval”-->Give Action Name as "Approval Action"-->select “Approval type” as “Manual approval”--> SNS topic ARN” select the SNS Topic that our reviewer is subscribed to i.e,"TriggerTopic"--> For “URL for review” 
